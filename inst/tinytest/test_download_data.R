...
if (requireNamespace("withr", quietly = TRUE)) {
  ...
  # A downloaded copy remains preferred only while it is at least as current
  # as the bundled snapshot.
  ...
  withr::with_envvar(c(R_USER_DATA_DIR = updated_user_data), {
    ...
    # A cache left by an older package version must not mask observations added
    # to a newer bundled snapshot.
    stale <- data.table::fread(downloaded)
    data.table::fwrite(stale[-nrow(stale)], downloaded, sep = "\t")
    expect_equal(grattanInflators:::available_series_id(series_id[1L]),
                 unname(bundled[1L]))
  })
}
